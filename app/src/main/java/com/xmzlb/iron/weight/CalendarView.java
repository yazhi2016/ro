...
//		canvas.drawPath(surface.boxPath, surface.borderPaint);
		...
		canvas.drawRect(0, 0,surface.width, surface.weekHeight, surface.bg);
	...
	private void calculateDate() {
		calendar.setTime(curDate);
		calendar.set(Calendar.DAY_OF_MONTH, 1);  //设置成1号
		int dayInWeek = calendar.get(Calendar.DAY_OF_WEEK); //1号是周几
		Log.d(TAG, "day in week:" + dayInWeek);  //1代表周日
		int monthStart = dayInWeek;
		if (monthStart == 1) { 
			monthStart = 8;  //如果1号是周日，则在第二行第一个显示首日，所以要加7
		}
		monthStart -= 2;  //以日为开头-1，以星期一为开头-2（如果是周一开头且1号周日，则1号在第一行第七个，data【6】，所以要减2）
		curStartIndex = monthStart; //这个月开始处
		date[monthStart] = 1;
		// last month
		if (monthStart > 0) {
			calendar.set(Calendar.DAY_OF_MONTH, 0); //设置成上个月的最后一天
			int dayInmonth = calendar.get(Calendar.DAY_OF_MONTH); //得到上个月最后一天是几号
		...
		showFirstDate = calendar.getTime(); //显示的第一个日期
		// this month
		calendar.setTime(curDate);
		...
		int monthDay = calendar.get(Calendar.DAY_OF_MONTH); //这个月总共多少天
		for (int i = 1; i < monthDay; i++) {
			date[monthStart + i] = i + 1;
		}
		curEndIndex = monthStart + monthDay; //下个月开始处
		// next month
	...
	private void drawCellBg(Canvas canvas, int index, int color) {
		...
		canvas.drawCircle(left + (surface.cellWidth - surface.borderWidth)/2, top + (surface.cellHeight- surface.borderWidth)/2,  surface.weekHeight/2, surface.cellBgPaint);
...
//		}
	...
	private class Surface {
		...
		public int cellDownColor = Color.parseColor("#00B700");
		public int cellSelectedColor = Color.parseColor("#00B700");
...
//		public String[] weekText = { "Sun","Mon", "Tue", "Wed", "Thu", "Fri", "Sat"};
		...
		public void init() {
			...
			bg = new Paint();
			bg.setColor(Color.parseColor("#D7D7D7"));
